...
namespace transport_catalogue::tests /* Prints */ {
    class Printer {
    public:
        template <class Map>
        static std::ostream& PrintMap(std::ostream& os, const Map& m) {
            os << "{";
            bool first = true;
            for (const auto& kv : m) {
                if (!first) {
        ...
        template <class Set>
        static std::ostream& PrintSet(std::ostream& os, const Set& s) {
            os << "{";
            bool first = true;
            for (const auto& x : s) {
                if (!first) {
...
namespace transport_catalogue::tests /* Stream operators override */ {
    template <class T>
...
namespace transport_catalogue::tests /* Asserts */ {
    /**
...
namespace transport_catalogue::tests /* TestRunner */ {
    class TestRunner {
    public:
...
namespace transport_catalogue::tests /* Asserts defs */ {
...
#define ASSERT_THROWS(expr, expected_exception)                                                                                    \
    {                                                                                                                              \
        ...
        if (!__assert_private_flag) {                                                                                              \
            std::ostringstream __assert_private_os;                                                                                \
            __assert_private_os << "Expression " #expr " is expected to throw " #expected_exception " " FILE_NAME ":" << __LINE__; \
            Assert(false, __assert_private_os.str());                                                                              \
        }                                                                                                                          \
    }
