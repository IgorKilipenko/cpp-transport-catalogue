...
namespace json::tests {
    ...
    public:
        ...
        void MustFailToLoad(const std::string& s) const {
            try {
                LoadJSON(s);
        ...
        void MustThrowLogicError(Fn fn) const {
            try {
                fn();
        ...
        void TestArray() const {
            ...
            [[maybe_unused]]const Array& arr = arr_node.AsArray();
            assert(arr.size() == 3);
        ...
        void TestMap() const {
            ...
            [[maybe_unused]]const Dict& dict = dict_node.AsMap();
            assert(dict.size() == 2);
        ...
        void TestJsonlib() const {
            ...
            */
            ...
            TestNumbers();
            std::cerr << prefix << "TestNumbers : Done." << std::endl;

            TestStrings();
            std::cerr << prefix << "TestStrings : Done." << std::endl;

            TestBool();
            std::cerr << prefix << "TestBool : Done." << std::endl;

            TestArray();
            std::cerr << prefix << "TestArray : Done." << std::endl;

            TestMap();
            std::cerr << prefix << "TestMap : Done." << std::endl;

            TestErrorHandling();
            std::cerr << prefix << "TestErrorHandling : Done." << std::endl;
