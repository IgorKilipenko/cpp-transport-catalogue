...
public:
    ...
    double ComputeSum(Date from, Date to) const {
        BudgetSpan b = tree_.ComputeSum(MakeDateSegment(from, to));
        return b.income - b.spendings;
    }
...
private:
    SummingSegmentTree<BudgetSpan, BulkLinearUpdater> tree_{GetDayIndex(END_DATE)};
};