...
/*
    ...
    void MapRenderer::RenderRouteNames(svg::Document& doc, const Routes& routes) const {
        ...
        for (const auto& route : routes) {
            if (route.second->stops.size() > 0) {
                ...
                doc.Add(text);
                if (route.second->route_type == domain::RouteType::LINEAR && route.second->stops.back() != route.second->stops.front()) {
                    text.SetPosition(FromLatLngToScreen(route.second->stops.back()->coordinate));
                    underlayer_text.SetPosition(FromLatLngToScreen(route.second->stops.back()->coordinate));
                    doc.Add(underlayer_text);
                    doc.Add(text);
                }
                ++color_index;
    ...
    void MapRenderer::RenderStops(svg::Document& doc, const Stops& stops, const BusesOnStops& buses_on_stops) const {
        for (const auto& stop : stops) {
            if (buses_on_stops.count(stop.first) != 0) {
                svg::Circle circle;
                circle.SetCenter(GetRelativePoint(stop.second->coordinate)).SetRadius(settings_.stop_radius).SetFillColor("white"s);
                doc.Add(circle);
            }
        }
    }
        ...
        for (const auto& stop : stops) {
            if (buses_on_stops.count(stop.first) != 0) {
                ...
                text.SetData(std::string(stop.first))
                    .SetPosition(GetRelativePoint(stop.second->coordinate))
    ...
    svg::Point MapRenderer::FromLatLngToScreen(geo::Coordinates coordinate) const {
        double zoom_coef;
        ...
        if (IsZero(field_width) && IsZero(field_height)) {
            ...
            zoom_coef = std::min((settings_.size.y - 2 * settings_.padding) / field_height, (settings_.size.x - 2 * settings_.padding) / field_width);
        }
    ...
    std::pair<geo::Coordinates, geo::Coordinates> MapRenderer::ComputeFieldSize(const transport_catalogue::TransportCatalogue& catalogue) const {
