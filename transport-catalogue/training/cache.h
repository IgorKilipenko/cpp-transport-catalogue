...
namespace trainig::sprint10 {
    ...
    public:
        ...
        explicit Cache(ValueFactoryFn value_factory) : cache_{}, factory_{value_factory} {
        ...
        std::shared_ptr<Value> GetValue(const Key& key) {
            std::weak_ptr<Value>& value = cache_[key];
            std::shared_ptr<Value> result;
            value = value.expired() ? std::weak_ptr<Value>(result = factory_(key)) : value;
            result = result ? result : value.lock();
            return result;
        }
    ...
    public:
        ...
        std::shared_ptr<Book> operator()(const std::string& title) const {
            const auto& book = store_.at(title);
            ++load_count_;
            return std::make_shared<Book>(title, book);
        }
...
namespace trainig::sprint10::cache::tests {
    inline void Test1() {
        using namespace std;
        ObjectPtr alice1;
