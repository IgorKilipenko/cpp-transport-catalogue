...
namespace trainig::sprint10::people::tests {

    using namespace std;

    // Симуляция жизни группы людей в течение num_steps дней
    inline void RunSimulation(vector<unique_ptr<Person>>& people, int num_days) {
    ...
    inline void Tests() {
        ...
        // Проверка полиморфного разрушения объекта Person
        {
            ...
            struct DestructivePerson : public Person {
                explicit DestructivePerson(bool& destruction_flag) : Person("test"s, 20), destruction_flag_(destruction_flag) {
...
namespace trainig::sprint10::people::example {
    ...
    inline int example_main() {
        ...
        tests::RunSimulation(people, 10);

        for (auto& person : people) {
            cout << person->GetName() << "' satisfaction: "s << person->GetSatisfaction() << ", dance count: "s << person->GetDanceCount() << endl;
        }
