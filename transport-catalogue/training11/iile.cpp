...
int main() {
    vector<Person> people = ReadPeople(std::cin);

    sort(people.begin(), people.end(), [](const Person& lhs, const Person& rhs) {
        return lhs.age < rhs.age;
    });

    for (string command; std::cin >> command;) {
        if (command == "AGE"s) {
            int adult_age;
            std::cin >> adult_age;

            auto adult_begin = lower_bound(people.begin(), people.end(), adult_age, [](const Person& lhs, int age) {
                return lhs.age < age;
            });

            cout << "There are "s << distance(adult_begin, people.end()) << " adult people for maturity age "s << adult_age << '\n';
            ...
            partial_sort(head.begin(), head.end(), people.end(), [](const Person& lhs, const Person& rhs) {
            ...
            IteratorRange range{people.begin(), partition(people.begin(), people.end(), [gender](Person& p) {
                                    if (gender == 'M') {
                                        return p.is_male;
                                    }
                                    return !p.is_male;
